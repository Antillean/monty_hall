...
class Game:
    def make_first_choice(self):
        """Randomly choose an index to make the initial choice."""
        return rand.choice([0, 1, 2])

    def run(self, i):
        """Run the game:

        1. Randomly initialise the choices.
        2. Randomly make the first choice and save its result.
        3. Make the switch choice and save its result.
        4. Make the random switch choice, and save its result.
        ...
        first_choice_value = choices[first_choice_index]

        switch_value = 0 if first_choice_value == 1 else 1
...
def get_num_runs_chunk(num_tries, chunk_size):
    ...
    so that the list of results doesn't get too big."""
